...
class Reports_model extends CI_Model
{
    ...
    public function relieved_table_booking_id($dateRange1='',$dateRange2='',$memeber_id='')
	{
		...
		$query="SELECT table_booking_id FROM ru_member_table_booking WHERE table_relieved_time BETWEEN '".$range1." 00:00:00' AND DATE_ADD('".$range2." 23:59:59', INTERVAL 4 HOUR)";
		
		// echo $dateRange2=date('Y-m-d').' 23:59:59';
	   ...
	   public function get_food_total_amount($relieved_arr)
	   {
	    ...
	    public function get_total_tables_booked($dateRange1='',$dateRange2='',$memeber_id=''){
			...
			if($dateRange1=='' || $dateRange2==''){
				...
				$sql="SELECT IFNULL(SUM((CHAR_LENGTH(table_booking_no) - CHAR_LENGTH(REPLACE(table_booking_no, ',', '')) + 1)),0) as total_table FROM `ru_member_table_booking` WHERE table_relieved_time BETWEEN '".$dateRange1."' AND DATE_ADD('".$dateRange2."', INTERVAL 4 HOUR)";
				// print($sql);
				if ( $memeber_id !="" ){
			...
			{
				$sql="SELECT IFNULL(SUM((CHAR_LENGTH(table_booking_no) - CHAR_LENGTH(REPLACE(table_booking_no, ',', '')) + 1)),0) as total_table FROM `ru_member_table_booking` WHERE table_relieved_time BETWEEN '".$dateRange1." 00:00:00' AND DATE_ADD('".$dateRange2." 23:59:59', INTERVAL 4 HOUR)";
	  ...
	  /* NUmber of guests */
	   ...
	   public function get_total_quests($dateRange1='',$dateRange2='',$memeber_id='')
	   {
		...
		if($dateRange1=='' || $dateRange2==''){
			...
			$sql="SELECT COUNT(DISTINCT(table_booking_id)) AS total FROM ru_member_table_booking WHERE table_relieved_time>='".$range1."' AND table_relieved_time<= DATE_ADD('".$range2."', INTERVAL 4 HOUR)";
			if ( $memeber_id !="" ){
				$sql .= " AND member_id ='$memeber_id'";
				}
		...
		else
		{
			...
			$sql="SELECT COUNT(DISTINCT(table_booking_id)) AS total FROM ru_member_table_booking WHERE table_relieved_time>='".$dateRange1." 00:00:00' AND table_relieved_time<= DATE_ADD('".$dateRange2." 23:59:59', INTERVAL 4 HOUR)";
			$res=$this->db->query($sql);
			if ( $memeber_id !="" ){
				$sql .= " AND member_id ='$memeber_id'";
				}
	   ...
	   public function get_total_tables($dateRange1='',$dateRange2='',$memeber_id=''){
		...
		if($dateRange1=='' || $dateRange2==''){
				$range1=date('Y-m-d').' 00:00:00';
				$range2=date('Y-m-d').' 23:59:59';
			$sql="SELECT SUM(no_of_guest) as total FROM `ru_member_table_booking` WHERE booking_time BETWEEN '".$range1."' AND DATE_ADD('".$range2."', INTERVAL 4 HOUR)";
			if ( $memeber_id !="" ){
				$sql .= " AND member_id ='$memeber_id'";
				}
			...
			$sql="SELECT SUM(no_of_guest) as total FROM `ru_member_table_booking` WHERE booking_time  BETWEEN '".$dateRange1." 00:00:00' AND DATE_ADD('".$dateRange2." 23:59:59', INTERVAL 4 HOUR)";
			$res=$this->db->query($sql);
			if ( $memeber_id !="" ){
				$sql .= " AND member_id ='$memeber_id'";
				}
	   ...
	   /*Service Delay Cases*/
	    public function get_delay_cases($relieved_arr){
		 ...
		 public function get_member_orders($dateRange1='',$dateRange2='',$memeber_id=''){
			...
			{
				$sql="SELECT m.id,m.membership_id,m.member_name,u.name as waiter_name,mt.no_of_guest,mt.booking_time,mt.table_booking_id,mt.cancel_reason,mt.member_status,fd.feedback_10,fd.text_feed_10,fd.cancel_reason AS feedback_cancel_reason,IFNULL(TIMESTAMPDIFF(MINUTE, mt.booking_time, mt.table_relieved_time),0) AS time_spent FROM `ru_member_table_booking` mt  INNER JOIN  `ru_membership` m ON m.id=mt.member_id INNER JOIN ru_users u ON u.user_id=mt.waiter_id LEFT JOIN ru_feedback fd ON mt.table_booking_id=fd.table_booking_id  WHERE table_relieved_time BETWEEN '".$dateRange1." 00:00:00' AND DATE_ADD('".$dateRange2." 23:59:59', INTERVAL 4 HOUR)";
