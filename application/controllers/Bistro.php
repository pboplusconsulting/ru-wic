...
class Bistro extends CI_Controller {
	...
	function __construct()
    {
	    ...
	    $this->load->model('Bistro_model');
		if($this->session->userdata('userRole')==3 || $this->session->userdata('userRole')==4 || $this->session->userdata('userRole')==5)
		{	
	...
	{//echo "<pre>";print_r($_SESSION);echo "</pre>";
		if(isset($_POST['complete_order_status']))
		{
            ...
            {
				$notificationToUsers=$this->Bistro_model->get_by_query("SELECT user_id FROM ru_users WHERE role!=4 AND status=1");
				$table_booking_id='';
				foreach($completeOrders as $order_id)
				{
					...
					$orderData=$this->Bistro_model->get_by_query_return_row($query);
			        $updateStatus=$this->Bistro_model->update_by_condition('ru_table_wise_order',array('order_status'=>1,'completed_time'=>date('Y-m-d H:i:s'),'modify_time'=>date('Y-m-d H:i:s')),array('table_order_id'=>$order_id));
					if($updateStatus)
					{
						...
						$this->Bistro_model->insert_one_row('ru_notifications',$notificationData1);
						...
						foreach($notificationToUsers as $notificationToUser)
						{
							...
							$insertid=$this->Bistro_model->insert_one_row('ru_notifications',$notificationData);
						}	
				...
				{	
				    $pendingOrders=$this->Bistro_model->no_of_pending_order($table_booking_id);
					if($pendingOrders==0)
					{	
				         $updateTableStatus=$this->Bistro_model->update_by_condition('ru_member_table_booking',array('member_status'=>3,'modify_time'=>date('Y-m-d H:i:s')),array('table_booking_id'=>$table_booking_id));
					}
				}	
        ...
        {			
			...
			************************************end*********************************/
			...
			$result=$this->Bistro_model->get_by_query($query);
	...
	public function search_member()
	{
		...
		if(!empty($search_string))
		{
			...
			$result=$this->Bistro_model->get_by_query($query);//echo "<pre>";print_r($result);echo "</pre>";die();
			//echo "<pre>";print_r($result);echo "</pre>";die();
			//$availableTables=$this->Bistro_model->leftjoin_orderby_and_result_array('ru_tables a','a.status=1 AND b.booking_timeb.table_status=0 ','a.id,a.table_name','ru_table_wise_booking b','a.id=b.table_booking_no','table_name','ASC')
			...
			//$availableTables=$this->Bistro_model->get_by_query($sql);
			//print_r($availableTables);die();
			$searchData['data']=$result;
			//$searchData['availableTables']=$availableTables;
			$html=$this->load->view('bistro/search_html',$searchData,true);
			echo $html;
		}
        else
        {
			...
			$result=$this->Bistro_model->get_by_query($query);//echo "<pre>";print_r($result);echo "</pre>";
			$searchData['data']=$result;
			//$searchData['availableTables']=$availableTables;
			$html=$this->load->view('bistro/search_html',$searchData,true);
			echo $html;
	...
	{//print_r($_POST);die();
		...
		{	
			...
			$orderData=$this->Bistro_model->get_by_query_return_row("SELECT * FROM ru_table_wise_order WHERE table_order_id=".$table_order_id);
			if($orderData!=null && $orderData->order_status==0)
			{	
			   $updateStatus=$this->Bistro_model->update_by_condition('ru_table_wise_order',array('order_status'=>2,'modify_time'=>date('Y-m-d H:i:s')),array('table_order_id'=>$table_order_id));
			   echo 1;
			}   
			else 
			{
	...
	*/
	...
	public function update_order_status()
	{
		...
		$resultAffected=$this->Bistro_model->update_by_condition('ru_table_wise_order',array('is_read'=>1),array('table_booking_id'=>$table_booking_id));
        if($resultAffected>0)
		   echo 1;
	    else
			echo 0;
	...
	public function order_cancel(){
		...
		{	
			...
			$resultAffected=$this->Bistro_model->update_by_condition('ru_table_wise_order',array('order_status'=>3,'cancel_reason'=>$comment,'cancel_time'=>date('Y-m-d H:i:s')),array('table_order_id'=>$table_order_id));
			...
			if($orderData!=null)
			{
				$message=$comment;
				//echo $message;die();
				$notificationToUsers=$this->Bistro_model->get_by_query("SELECT user_id FROM ru_users WHERE role!=5 AND status=1");
				foreach($notificationToUsers as $notificationToUser)
				{
					$notificationData=array(
											'from_user_id'=>$this->session->userdata('userID'),
