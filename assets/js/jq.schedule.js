...
(function($) {
	...
	var ak = 0;
        ...
        this.addScheduleBgData = function(data){
			...
			var bgcolor = element.formatTime(data["bgcolor"]);
        ...
        this.addScheduleData = function(data){
            var st = Math.ceil((data["start"] - tableStartTime) / setting.widthTime);
            var et = Math.floor((data["end"] - tableStartTime) / setting.widthTime);
            var $bar = jQuery('<div class="sc_Bar"><!--<span class="head"><span class="time"></span>--></span><span class="text"></span><span class="btext"></span></div>');
            var stext = element.formatTime(data["start"]);
            var etext = element.formatTime(data["end"]);
			...
			if(data["Btext"]){
                $bar.find(".btext").text(data["Btext"]);
            }
            if(data["class"]){
                $bar.addClass(data["class"]);
            }
        ...
        this.addRow = function(timeline,row){
            ...
            if(row["schedule"]){
                for(var i in row["schedule"]){
                    ...
                    data["end"] = e;
                    if(bdata["bgcolor"]){
                        data["bgcolor"] = bdata["bgcolor"];
                    }
					if(bdata["Btext"]){
                        data["Btext"] = bdata["Btext"];
                    }
					if(bdata["text"]){
        ...
        this.resizeRow = function(n,height){
            ...
            $element.find(".sc_data").height(100+'%');
			$element.find(".sc_data").css('min-height',($element.find(".sc_main_box").height()));
        }
        // resizeWindow
                ...
                if(
                    (before_time < 0) ||
                        (Math.floor(before_time / 3600) != Math.floor(t / 3600))){
							ak++;
                    var html = '';
                    html += '<div class="sc_time">'+ak+' Day</div>';
